    ...
    if (p.type=='nplot') {
        ...
        if (is.null(graph.simi$com)) {
            ...
            if (graph.simi$halo) {
                ...
                n.groups <- length(mark.groups)
                if (n.groups > 0) {
                    base.cols <- del.yellow(rainbow(n.groups))
                    mark.col <- sapply(base.cols, function(col) adjustcolor(col, alpha.f = 0.24))
                    mark.border <- sapply(base.cols, function(col) adjustcolor(col, alpha.f = 0.58))
                }
            } else {
                mark.groups <- NULL
            }
            plot(com, g.toplot,vertex.label='', edge.width=we.width, vertex.size=vertex.size, vertex.color=vertex.col, vertex.label.color='white', edge.label=we.label, edge.label.cex=cex, edge.color=edge.col, vertex.label.cex = 0, layout=lo, mark.groups = mark.groups, mark.col = mark.col, mark.border = mark.border, edge.curved=edge.curved)
        }
        #txt.layout <- lo
